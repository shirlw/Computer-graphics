...
inline float lerp(float x, float y, float t) {
    /// TODONE: Implement linear interpolation between x and y
    //return 0;
    return x+t*(y-x);
}
...
R32FTexture* fBm2DTexture() {
    ...
    float *perlin_data = perlin2D(width, height,256);// 128); //32 for more noise
    ...
    for (int i = 0; i < octaves; ++i) {
        ...
        /*computer weight for each frequency*/
